...
=head2 F<execstate_api.md>

Specification of a core API that creates, swaps and tears down a green-thread
execution state - the per-thread interpreter state a stackful coroutine module
has to switch - so that such a module need not reach into interpreter internals
to do it.
...
=head2 F<green_threads_and_execstate.md>

A study of the concurrency models available to Perl - ithreads, process pools,
and stackful and stackless green threads - and the case it makes for the
execstate, parkapi and multicore APIs documented alongside it.
...
=head2 F<perlmulticore_xs_example.md>

The same XSUB in three forms - unported, bracketed with release/acquire, and
restructured for offload - as a worked example of porting an XS module to the
multicore hook.
...
=head2 F<savestack_suspend_api.md>

Specification of a core API that freezes, unwinds and later re-applies a region
of the save stack, so that a module suspending a scope need not reimplement
leave_scope in reverse.
